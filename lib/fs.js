...
// 3rd party modules
const logger = require('oniyi-logger')('oniyi:utils:fs');
...
function isDirectory(checkPath) {
  try {
    ...
    logger.debug('got an exception when checking if "%s" is a directory', checkPath, isDirectoryError);
    return false;
  }
}
...
function safeReaddir(...args) {
  try {
    ...
    logger.debug('failed to `fs.readdirSync()` with args %j', args, tryReadDirError);
    return [];
  }
}
